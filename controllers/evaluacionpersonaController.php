...
class evaluacionpersonaController extends Controller
{
	...
	public function index()
	{
		if (isset($_SESSION['usuario'])) {
			...
			$this->_view->conctructor_menu('home','evaluacionpersona');
			...
			$this->_view->setJs_Specific(
				array(
					...
					'plugins/highcharts10/highcharts',
					'plugins/highcharts10/modules/variable-pie',
					'plugins/highcharts10/highcharts-more',
				)
			);
			...
			$result = $soap->ListadoEvaluacion($param);
	...
	// public function ver_pdf_evaluacion()
	{
		if (isset($_SESSION['usuario'])) {
			...
			if (count($ListadoEvaluacionPdf) > 0) {
				...
				// $graph1->Stroke();
				$graph->Stroke("temp/".$dia2 . ".png");
				$graph1->Stroke("temp/".$dia3 . ".png");
				$graph2->Stroke("temp/".$dia4 . ".png");

				$pdf->Image("temp/".$dia2 . ".png", 18, 115, 80, 72);
				$pdf->Image("temp/".$dia3 . ".png", 100, 115, 95, 72);
				$pdf->Image("temp/".$dia4 . ".png", 50, 190, 120, 65);
