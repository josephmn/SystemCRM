...
class conremotoController extends Controller
{
	...
	public function index()
	{
		if (isset($_SESSION['usuario'])) {
			...
			$this->_view->conctructor_menu('controlper','conremoto');
			...
			$result4 = $soap->ControlRemoto($param3);
			$controltbremoto = json_decode($result4->ControlRemotoResult, true);

			// construimos la tabla
			$filas="";
			foreach($controltbremoto as $tb){
	...
	public function mantenimiento_remoto()
	{
		if (isset($_SESSION['usuario'])) {
			...
			$result = $soap->MantControlRemoto($params);
			$tbcontrolremoto= json_decode($result->MantControlRemotoResult, true);

			header('Content-type: application/json; charset=utf-8');

			echo $json->encode(
				array(
					"vicon" 		=> $tbcontrolremoto[0]['v_icon'],
					"vtitle" 		=> $tbcontrolremoto[0]['v_title'],
					"vtext" 		=> $tbcontrolremoto[0]['v_text'],
					"itimer" 		=> $tbcontrolremoto[0]['i_timer'],
					"icase" 		=> $tbcontrolremoto[0]['i_case'],
					"vprogressbar" 	=> $tbcontrolremoto[0]['v_progressbar'],
				)
			);
